...
/**
 * Provides a 'bc Content Overview'.
 *
 * @Block(
 * id = "bc_content_overview",
 * admin_label = @Translation("My Site Overview"),
 * category = @Translation("Dashboard")
 * )
 */
class BcContentOverview extends BlockBase implements BlockPluginInterface {
  ...
  public function build() {
    ...
    $spam = isset($config['bc_total_control_spam_overview']) && $config['bc_total_control_spam_overview'] == 1;

    foreach ($types as $type => $object) {
      // Compare against type option on pane config.
      if ((!array_key_exists($type, $config['bc_total_control_types_overview'])) || (isset($config['bc_total_control_types_overview']) && $config['bc_total_control_types_overview'][$type]) == $type) {
        ...
        if ($comments_exist) {
          ...
          if ((!array_key_exists($type, $config['bc_total_control_comments_overview'])) || (isset($config['bc_total_control_comments_overview']) && $config['bc_total_control_comments_overview'][$type]) == $type) {
  ...
  public function blockForm($form, FormStateInterface $form_state) {
    ...
    $form['bc_total_control_types_overview'] = [
      '#type' => 'checkboxes',
      '#title' => $this->t('Show post counts for the following content types'),
      '#options' => $type_defaults,
      '#default_value' => $config['bc_total_control_types_overview'],
    ];

    $form['bc_total_control_comments_overview'] = [
      '#type' => 'checkboxes',
      '#title' => $this->t('Show comment counts for the following content types'),
      '#options' => $type_defaults,
      '#default_value' => $config['bc_total_control_comments_overview'],
    ];
    $spam_options = [
      0 => t('no'),
      1 => t('Yes'),
    ];
    $form['bc_total_control_spam_overview'] = [
      '#type' => 'radios',
      '#title' => $this->t('Include spam counts with comments'),
      '#options' => $spam_options,
      '#default_value' => $config['bc_total_control_spam_overview'],
    ];
    return $form;
  }
  ...
  public function blockSubmit($form, FormStateInterface $form_state) {
    ...
    $this->configuration['bc_total_control_types_overview'] = $values['bc_total_control_types_overview'];
    $this->configuration['bc_total_control_comments_overview'] = $values['bc_total_control_comments_overview'];
    $this->configuration['bc_total_control_spam_overview'] = $values['bc_total_control_spam_overview'];
  }
